...
import com.starbook.core.featureflag.MemoryFeatureFlag
...
class SettingsViewModelTest {
  ...
  private val themeColorSchemeStore = MemoryDataStore(ThemeColorScheme.StarBookBlue)
  ...
  @Test
  fun `view state defaults to follow system and StarBook blue`() = scope.runTest {
    backgroundScope.launchMolecule(RecompositionMode.Immediate) {
      viewModel.viewState()
    }.test {
      awaitItem().let {
        assertEquals(expected = ThemeMode.FollowSystem, actual = it.themeMode)
        assertEquals(expected = ThemeColorScheme.StarBookBlue, actual = it.themeColorScheme)
      }
    }
  }
  ...
  fun `selecting dynamic color updates view state`() = scope.runTest {
    backgroundScope.launchMolecule(RecompositionMode.Immediate) {
      ...
      assertEquals(expected = ThemeColorScheme.StarBookBlue, actual = awaitItem().themeColorScheme)
  ...
  @Test
  fun `openSupportStarBook navigates to support screen`() {
    viewModel.openSupportStarBook()
