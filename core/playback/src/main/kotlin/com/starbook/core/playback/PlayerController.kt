...
import com.starbook.core.playback.session.CustomCommand
import com.starbook.core.playback.session.MediaItemProvider
import com.starbook.core.playback.session.PlaybackService
import com.starbook.core.playback.session.bookId
import com.starbook.core.playback.session.playbackItemForPosition
import com.starbook.core.playback.session.positionInMediaItem
import com.starbook.core.playback.session.sendCustomCommand
...
class PlayerController(
  ...
  fun play(id: BookId? = null) = executeAfterPrepare(id) { controller ->
    controller.play()
  }
  ...
  fun playPause(id: BookId? = null) = executeAfterPrepare(id) { controller ->
  ...
  private suspend fun maybePrepare(
    controller: MediaController,
    bookIdOverride: BookId? = null,
  ): Boolean {
    val bookId = bookIdOverride ?: currentBookStoreId.data.first() ?: return false
    ...
    controller.setMediaItem(mediaItemProvider.toMediaItem(book))
    controller.prepare()
    return true
  }
  ...
  private inline fun executeAfterPrepare(
    bookIdOverride: BookId? = null,
    crossinline action: suspend (MediaController) -> Unit,
  ) {
    scope.launch {
      val controller = awaitConnect() ?: return@launch
      if (maybePrepare(controller, bookIdOverride)) {
        action(controller)
      }
    }
  }
