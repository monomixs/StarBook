package com.starbook.core.scanner.mp4
...
import com.starbook.core.scanner.mp4.visitor.ChapVisitor
import com.starbook.core.scanner.mp4.visitor.ChplVisitor
import com.starbook.core.scanner.mp4.visitor.Co64Visitor
import com.starbook.core.scanner.mp4.visitor.MdhdVisitor
import com.starbook.core.scanner.mp4.visitor.StcoVisitor
import com.starbook.core.scanner.mp4.visitor.StscVisitor
import com.starbook.core.scanner.mp4.visitor.SttsVisitor
...
internal class Mp4BoxParser(
  ...
  private fun parseBoxes(
    ...
    while (input.position < parentEnd) {
      ...
      when {
        visitor != null -> {
          ...
          if (payloadSize > Int.MAX_VALUE) {
            Logger.w("Atom ${visitor.path.last()} is too large to read (> 2GB)")
            if (!skipFullyLong(input, payloadSize)) return
          } else {
            val payloadSizeInt = payloadSize.toInt()
            scratch.reset(payloadSizeInt)
            if (!input.readFully(scratch.data, 0, payloadSizeInt, true)) {
              return
            }
        ...
        visitors.any { it.path.startsWith(currentPath) } -> {
          ...
          if (!skipFullyLong(input, payloadSize)) {
            return
          }
        }
      }
      ...
      if (input.position < payloadEnd) {
        if (!skipFullyLong(input, payloadEnd - input.position)) {
          return
        }
      }
    }
  ...
  private fun skipFullyLong(input: ExtractorInput, bytesToSkip: Long): Boolean {
