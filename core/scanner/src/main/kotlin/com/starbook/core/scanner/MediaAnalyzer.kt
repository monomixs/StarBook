...
import com.starbook.core.scanner.matroska.MatroskaMetaDataExtractor
import com.starbook.core.scanner.matroska.MatroskaParseException
import com.starbook.core.scanner.mp4.Mp4ChapterExtractor
...
internal class MediaAnalyzer(
  ...
  suspend fun analyze(file: CachedDocumentFile): Metadata? {
    ...
    if (duration == null || duration <= Duration.ZERO) {
      Logger.w("Duration retrieval failed or returned zero for file: ${file.uri}")
      return null
    }
    ...
    if (trackGroups != null) {
            ...
            format.metadata?.let { metadata ->
              repeat(metadata.length()) { metadataIndex ->
                when (val entry = metadata.get(metadataIndex)) {
                  ...
                  else -> Logger.v("Unknown metadata entry: $entry")
                }
              }
            }
          }
      ...
      Logger.i("MetadataRetriever failed for ${file.uri}, falling back to custom parsers.")
    }
  ...
  private suspend fun retrieveMetadata(uri: Uri): TrackGroupArray? {
    return try {
      ...
      Logger.w(e, "Error retrieving metadata for $uri")
      null
    }
  }

  private suspend fun retrieveDuration(uri: Uri): Duration? {
    // Primary: Use Media3 MetadataRetriever
    val duration = try {
      MetadataRetriever.Builder(context, MediaItem.fromUri(uri))
        .setMediaSourceFactory(mediaSourceFactory)
        .build()
        .use {
          ...
          if (durationUs > 0) durationUs.microseconds else null
        }
    } catch (e: Exception) {
      if (e is CancellationException) currentCoroutineContext().ensureActive()
      null
    ...
    // Fallback: Use platform MediaMetadataRetriever for very large or complex files
    val retriever = MediaMetadataRetriever()
    return try {
      retriever.setDataSource(context, uri)
      val time = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DURATION)
      time?.toLongOrNull()?.milliseconds
    } catch (e: Exception) {
      Logger.w(e, "Fallback duration retrieval failed for $uri")
      null
    } finally {
      try {
        retriever.release()
